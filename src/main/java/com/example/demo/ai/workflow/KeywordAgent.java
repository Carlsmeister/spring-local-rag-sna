...
public class KeywordAgent {
    ...
    @Value("classpath:prompts/keyword-agent-system-prompt.txt")
    private Resource systemPromptResource;

    public KeywordAgent(ChatClient.Builder chatClientBuilder) {
        this.chatClient = chatClientBuilder.build();
    }

    public AgentResponse analyze(String sanitizedText) {
        if (sanitizedText == null || sanitizedText.isBlank()) {
            return AgentResponse.success("Keyword Agent Report: No CV text provided.", 0L, 0L, 0L);
        }
        ...
        try (var inputStream = systemPromptResource.getInputStream()) {
            ...
            return AgentResponse.failure("Failed to read Keyword agent system prompt: " + e.getMessage());
        }
        ...
        try {
            ...
            return AgentResponse.failure("Keyword Agent LLM call failed: " + e.getMessage());
        }
    }
}
