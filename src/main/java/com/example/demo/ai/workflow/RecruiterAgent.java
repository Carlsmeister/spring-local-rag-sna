...
public class RecruiterAgent {
    ...
    @Value("classpath:prompts/recruiter-agent-system-prompt.txt")
    private Resource systemPromptResource;

    public RecruiterAgent(ChatClient.Builder chatClientBuilder) {
        this.chatClient = chatClientBuilder.build();
    }

    public AgentResponse analyze(String sanitizedText) {
        if (sanitizedText == null || sanitizedText.isBlank()) {
            return AgentResponse.success("Recruiter Agent Report: No CV text provided.", 0L, 0L, 0L);
        }
        ...
        try (var inputStream = systemPromptResource.getInputStream()) {
            ...
            return AgentResponse.failure("Failed to read Recruiter agent system prompt: " + e.getMessage());
        }
        ...
        try {
            ...
            return AgentResponse.failure("Recruiter Agent LLM call failed: " + e.getMessage());
        }
    }
}
